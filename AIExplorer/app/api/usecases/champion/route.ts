...
export const GET = async (request: Request): Promise<NextResponse> => {
  ...
  try {
    ...
    const result = await pool
      .request()
      .input("StakeholderEmail", email)
      .execute("dbo.GetChampionUseCases");
    ...
    logErrorTrace("Champion usecases failed", error);
    return NextResponse.json(
      {
        message: getUiErrorMessage(error, "Failed to load champion use cases."),
      },
      { status: 500 },
    );
  }
